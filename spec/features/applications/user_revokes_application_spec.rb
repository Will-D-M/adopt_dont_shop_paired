...
RSpec.describe 'application revoked' do
  before(:each) do
  ...
  describe "I visit pet show page after approved applications" do
    scenario "I see unapprove link" do
      ...
      visit "/applications/#{@application.id}"
      expect(page).to have_link("Unapprove Patra's application")
      click_link("Unapprove Patra's application")
    end

    scenario "click unapprove link, go to show page, approve link is back" do
      @application = Application.all.first
      visit "/applications/#{@application.id}"
      click_link("Approve Patra's application")
      ...
      expect(page).not_to have_content('This pet is on hold for Heihachi.')
    end
